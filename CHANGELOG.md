...
### Fixed
- The "70+ more" badge sat next to nine named agents, which implied 79, and a bare count told the reader nothing anyway. The counts are gone. Zed, Warp, Devin and Amp joined the badge row, and a new "Every agent you can install into" section names all 76 values the installer accepts after `-a`, so you can find yours instead of trusting a number. The list was read out of the installer, not estimated.
