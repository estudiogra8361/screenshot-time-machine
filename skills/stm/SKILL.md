...
2. If the output is `{"error":"no_server", ...}`, start the app (usually `npm run dev`) or ask which port it is on, then rerun with `--url http://localhost:<port>`. If it is `no_browser`, the machine needs Chrome or Edge, or a one-time download of a headless browser (about 120 MB); the message says which.
